...
export function TabelogGmapPanel() {
  const [settings, setSettings] = useState<TabelogGmapSettings>(DEFAULT_SETTINGS);
      ...
      <Card>
        <CardContent className="flex items-start gap-3 p-3">
          ...
          <div className="min-w-0 flex-1">
            <h3 className="text-fg-default text-sm font-semibold">食べログ × Google Maps</h3>
            <p className="text-fg-subtle mt-1 text-[11px] leading-relaxed">
              食べログの店舗詳細ページに、その店の Google Maps 評価カードを自動で挿入します。
              バックグラウンドで Maps を一瞬開いて評価を取得 → 7 日間キャッシュ。
            </p>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="border-border bg-surface-elevated flex items-center justify-between rounded-md border px-3 py-2.5">
        <div className="min-w-0 flex-1">
          ...
          <p className="text-fg-subtle mt-0.5 text-[11px]">
            tabelog.com/*/A*/A*/数字/ の店舗ページでのみ動作します。
          </p>
        </div>
      ...
      <label className="border-border bg-surface-elevated flex items-start gap-3 rounded-md border p-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="text-fg-default text-sm font-medium">
            取得失敗時に「Maps で検索」リンクを出す
          </div>
          <p className="text-fg-subtle mt-0.5 text-[11px]">
            CAPTCHA / マッチング失敗時、手動検索リンクのみ表示します。
          </p>
        </div>
      </label>
    </div>
